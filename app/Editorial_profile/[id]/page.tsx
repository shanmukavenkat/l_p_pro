...
export default function ProfileDetail({ params }: { params: Promise<{ id: string }> }) {
  // 1. MANDATORY: Unwrap params using React.use() for Next.js 15
 ...
 if (loading) return (
  ...
  // Constructing URL
  const s3ImageUrl = `https://lurnexa.s3.ap-south-1.amazonaws.com/editorial_board_photos/${id}.${extensions[imgExtensionIndex]}`;
  const fallbackImage = `https://ui-avatars.com/api/?name=${encodeURIComponent(member.name || "User")}&background=6366f1&color=fff&size=300`;
