        ...
        <main className="flex flex-col lg:flex-row gap-6 md:gap-8 p-4 md:p-8 lg:p-12 max-w-7xl mx-auto">
          ...
          <aside className="w-full lg:w-1/3 xl:w-1/4 h-fit bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-3xl p-6 md:p-8 text-center shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs md:text-sm font-medium text-slate-500 break-all bg-slate-50 py-1 px-3 rounded-full inline-block max-w-full">
                {formData.email}
              </p>
              <div className="pt-2">
                <span className="text-[9px] md:text-[10px] font-bold tracking-widest text-slate-400 uppercase bg-white border border-slate-100 px-2 py-0.5 rounded">
                  ID: {formData.id}
                </span>
              </div>
            </div>
          ...
          <section className="flex-1 bg-white border border-slate-200/60 rounded-3xl p-5 md:p-10 shadow-sm">
            <div className="flex items-center justify-between mb-6 md:mb-8 border-b border-slate-100 pb-4 md:pb-6">
              <h3 className="text-lg md:text-xl font-bold text-slate-900 flex items-center gap-2 md:gap-3">
                <div className={`p-2 rounded-lg ${isEditing ? 'bg-indigo-50' : 'bg-slate-50'}`}>
                  {isEditing ? <Pencil size={18} className="text-indigo-600" /> : <User size={18} className="text-slate-600" />}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-x-8 md:gap-y-6">
              {[
                ["name", "Full Name"], ["role", "Professional Role"], ["college_id", "Institutional ID"],
                ["organization_name", "Organization"], ["contact_no", "Phone Number"], ["list_of_items", "Area of Expertise"],
                ["recent_work", "Latest Projects"], ["ongoing_work", "Active Research"], ["description", "Biography / Summary"],
              ].map(([key, label]) => (
                <div key={key} className="space-y-1.5 md:space-y-2">
                  <label className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">
                    {label}
                  </label>
                  <Input
                    name={key}
                    ...
                    disabled={!isEditing}
                    className={`h-10 md:h-12 px-4 rounded-xl transition-all ${isEditing ? 'bg-white border-slate-200' : 'bg-slate-50/50 border-transparent cursor-not-allowed'}`}
                  />
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 md:mt-8 space-y-2">
              ...
              <textarea
                ...
                onChange={(e) => e.target.value.length <= 500 && handleChange(e as any)}
                disabled={!isEditing}
                rows={4}
                className={`w-full px-4 py-3 rounded-xl transition-all resize-none text-sm md:text-base ${isEditing ? "bg-white border-slate-200" : "bg-slate-50/50 border-transparent cursor-not-allowed"}`}
              />
              <div className="text-right text-[10px] md:text-xs text-slate-400">
                {(formData as any).list_of_items?.length || 0} / 500
              </div>
            </div>
                ...
                {formData.resume && (
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 md:p-4 bg-white border border-slate-200 rounded-xl gap-3">
                    <div className="flex items-center gap-3 md:gap-4">
                      <div className="h-10 w-10 md:h-12 md:w-12 bg-red-50 rounded-lg flex items-center justify-center text-red-500"><FileText size={20} /></div>
                      <div>
                        <p className="text-xs md:text-sm font-bold text-slate-900 truncate max-w-[150px] sm:max-w-none">Research_Resume.pdf</p>
                        <p className="text-[9px] md:text-[10px] font-medium text-slate-400 uppercase">PDF</p>
                      </div>
                    </div>
                    <a href={typeof formData.resume === 'string' ? formData.resume : URL.createObjectURL(formData.resume)} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto">
                      <Button variant="ghost" size="sm" className="w-full text-indigo-600 text-xs md:text-sm"><ExternalLink size={14} className="mr-2" /> View</Button>
                    </a>
                  </div>
                )}
                {isEditing && (
